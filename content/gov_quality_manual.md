...
### 5.3.1 제품 분야 품질지표 (Product Quality Measures)
...
| NO | 품질특성 | 측정 지표명 | 측정 지표 정의 |
|----|---------|------------|---------------|
| 1 | 기능 적합성 | 기능 요구사항 반영율 | 사용자가 요구한 기능이 시스템에 반영되었는가를 측정하는 지표 |
| 2 | 기능 적합성 | 기능 요구사항 정확성 | 사용자가 요구한 기능이 시스템에 올바르게 반영되었는가를 측정하는 지표 |
| 3 | 성능 효율성 | 응답시간(평균응답시간) | 사용자가 온라인 화면에서 데이터를 입력 후 클릭한 시간부터 결과가 표출된 시간 |
| 4 | 성능 효율성 | CPU 사용율 | 할당된 작업을 구현하는데 사용된 CPU 시간 |
| 5 | 성능 효율성 | 메모리 사용율 | 할당된 작업을 구현하는데 사용된 메모리 공간 |
| 6 | 양립성 | 데이터 교환성 | 타 시스템 간 또는 서브시스템 간에 송수신되는 데이터의 무결성을 측정하기 위한 지표 |
| 7 | 사용성 | 학습 용이성 | 사용자가 매뉴얼 및 산출물에 대한 이해의 용이성을 측정하는 지표 |
| 8 | 사용성 | 메시지 명료성 | 시스템에 구현된 메시지들이 얼마나 쉽게 이해되는지 정도를 측정하는 지표 |
| 9 | 신뢰성 | 평균무고장시간(MTBF) | 고장이 복구된 시점에서 다음 고장 발생까지의 평균 연속 가동시간(무고장 동작시간) |
| 10 | 신뢰성 | 평균회복시간 | 시스템의 고장을 복구하는데 소요되는 평균 시간 |
| 11 | 신뢰성 | 결함 허용성 | 시스템에서 결함이 발생될 경우 사용자요구사항정의서에 명시된 수준으로 성능을 유지하는 정도를 측정하는 지표 |
| 12 | 보안성 | 데이터 암호화 | 요구사항 명세서에 정의된 데이터 암호화 또는 해독이 정확하게 구현되는지를 측정하는 지표 |
| 13 | 보안성 | 접근 감사성 | 시스템과 데이터에 사용자 접근과 관련된 감사추적의 완전성을 측정하는 지표 |
| 14 | 유지보수성 | 수정복잡도 | 유지보수자가 문제해결을 위해 소프트웨어를 쉽게 수정할 수 있는 정도를 측정하는 지표 |
| 15 | 이식성 | 사용자 지원기능 일관성 | 새로운 구성요소가 현재의 사용자 인터페이스와 일관적인지를 측정하는 지표 |

### 5.3.2 프로세스 분야 품질지표 (Process Quality Measures)
...
단위활동 수준의 품질관리 대상은 "CBD SW개발 표준산출물 관리가이드"(2011.12, 한국정보화진흥원)에서 제시된 공정단계별 개발 표준산출물인 총 25종을 대상으로 합니다.

| 단계 | 개발 표준 산출물 | 산출물 종수 |
|------|----------------|------------|
| 분석단계 | 사용자 요구사항 정의서, 유스케이스 명세서, 요구사항 추적표 | 3 |
| 설계단계 | 클래스 설계서, 사용자 인터페이스 설계서, 컴포넌트 설계서, 인터페이스 설계서, 아키텍처 설계서, 총괄시험 계획서, 시스템 시험 시나리오, 엔티티 관계 모형 기술서, 데이터베이스 설계서, 통합시험 시나리오, 단위시험 케이스, 데이터 전환 및 초기데이터 설계서 | 12 |
| 구현단계 | 프로그램 코드, 단위시험 결과서, 데이터베이스 테이블 | 3 |
| 시험단계 | 통합시험 결과서, 시스템시험 결과서 | 2 |
| 전개단계 | 사용자 지침서, 운영자 지침서, 시스템 설치 결과서, 인수시험 시나리오, 인수시험 결과서 | 5 |
